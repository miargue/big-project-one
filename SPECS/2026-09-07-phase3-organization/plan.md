...
- [x] 1.1 Write failing tests in `tests/test_organization.py`:
...
- [x] 1.2 Implement the organization panel in `app.py`:
...
- [x] 1.3 Add the organization panel HTML to `templates/index.html`:
...
- [x] 1.4 Check: run the tests until green.

## Task Group 2: Date Range Filter

- [x] 2.1 Write failing tests for date range filtering:
...
- [x] 2.2 Implement date range filter in `app.py`:
...
- [x] 2.3 Add date range inputs to `templates/index.html`:
      - Start date input field.
      - End date input field.
      - "Apply filter" button.
- [x] 2.4 Check: run all tests until green.

## Task Group 3: Specification & Cleanup

- [x] 3.1 Run the full test suite; all green.
- [x] 3.2 Compare the implementation against these feature specs and the constitution;
      note any differences for the validation step.
- [x] 3.3 Update SPECS/ROADMAP.md current state to reflect Phase 3 completion.
- [x] 3.4 On the user's approval, update any constitution or spec files to match
